...
function mits_copy_option_with_values(int $oID): void
{
    $oID = (int)$oID;
    ...
    $options_query = xtc_db_query("SELECT * FROM " . TABLE_PRODUCTS_OPTIONS . " WHERE products_options_id = " . xtc_db_prepare_input($oID));
    ...
    $values_query_raw = "SELECT pov.*
                        ...
                        WHERE pov2po.products_options_id = " . xtc_db_prepare_input($oID);
...
if (defined('FILENAME_PRODUCTS_ATTRIBUTES') && isset($PHP_SELF) && basename($PHP_SELF) == FILENAME_PRODUCTS_ATTRIBUTES
  && isset($_GET['action']) && $_GET['action'] == 'copy_option_and_values'
  && isset($_GET['oID']) && !empty($_GET['oID']) && isset($messageStack)
) {
    mits_copy_option_with_values($_GET['oID']);
    $messageStack->add_session(sprintf(MITS_COPY_OPTION_WITH_VALUES_TEXT_SUCCESS, $_GET['oID']), 'success');
