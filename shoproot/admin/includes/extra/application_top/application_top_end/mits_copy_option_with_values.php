...
function mits_copy_option_with_values($oID) {
  ...
  $options_query = xtc_db_query("SELECT * FROM " . TABLE_PRODUCTS_OPTIONS . " WHERE products_options_id = " . (int)$oID);
  ...
  $values_query_raw = "SELECT pov.*
                        ...
                        WHERE pov2po.products_options_id = " . (int)$oID;
...
if (basename($PHP_SELF) == FILENAME_PRODUCTS_ATTRIBUTES
  ...
  if (is_file(DIR_FS_DOCUMENT_ROOT . (defined('DIR_ADMIN') ? DIR_ADMIN : 'admin/') . 'includes/extra/css/mits_copy_option_with_values.php')) {
    @unlink(DIR_FS_DOCUMENT_ROOT . (defined('DIR_ADMIN') ? DIR_ADMIN : 'admin/') . 'includes/extra/css/mits_copy_option_with_values.php');
  }
